...
sed -i '/<uses-permission android:name="android.permission.FOREGROUND_SERVICE"\/>/d' "$manifest" || true
...
git add -A
git commit -m "fix: ML Kit OCR, accessibility config, FG service; SDK 34; CI workflow" || true
git push -u origin "$branch" || true

echo "OPEN_PR: https://github.com/$(git config --get remote.origin.url | sed -E 's#.*github.com[:/](.+/.+)(\.git)?#\1#')/compare/$branch?expand=1"
